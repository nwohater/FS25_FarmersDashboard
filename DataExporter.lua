...
function DataExporter:getFieldsLive()
    ...
    for _, field in pairs(g_fieldManager.fields) do
      ...
      "farmId": %d,
      "farmName": "%s"
    }]], field.fieldId or -1, self:escape(fruitTypeName), growthState or 0, self:escape(growthStateLabel), field.areaHa or 0, sprayLevel, limeLevel, farmId, self:escape(farmName)))
        end
    end
